...
$(function() {
	load();
	debugger;
	$('.bootstrap-table .pull-right').append('<div class="keep-open btn-group" title="列"><button type="button" class="btn btn-default btn-outline dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="glyphicon glyphicon-search"></i> <span class="caret"></span></button><ul class="dropdown-menu colName-select" role="menu"></ul></div>');
	 var colLength = $('#exampleTable thead tr th').length - 2;
	 for(var i=0;i<colLength;i++){
		 $('.bootstrap-table .pull-right .colName-select').append('<li></li>');
		 var colName = $('#exampleTable thead tr th:not(:first-child):not(:last-child)').eq(i).text();
		 $('.bootstrap-table .pull-right .colName-select li').eq(i).text(colName);
	 }	
});
