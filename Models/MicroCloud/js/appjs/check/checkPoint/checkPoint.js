...
var prefix2 = contpath+"check/checkPoint"
$(function() {
	//load2();
});
...
function reLoad2() {
	$('#chekline-point').bootstrapTable('refresh');
}
function add2() {
	layer.open({
		type : 2,
...
function batchRemove2() {
	var rows = $('#chekline-point').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
