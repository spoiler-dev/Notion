...
var prefix3 = contpath+"check/checkTaskItem"
$(function() {
	//load3();
});
...
function reLoad3() {
	$('#checkTaskLine-item').bootstrapTable('refresh');
}
function add3() {
	layer.open({
		type : 2,
...
function batchRemove3() {
	var rows = $('#checkTaskLine-item').bootstrapTable('getSelections'); // 返回所有选择的行，当没有选择的记录时，返回一个空数组
	if (rows.length == 0) {
		layer.msg("请选择要删除的数据");
		return;
	}
